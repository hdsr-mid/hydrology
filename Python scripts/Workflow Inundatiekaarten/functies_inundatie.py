...
def add_water_to_raster(input_file, output_file, water_level):       
    ...
    with rasterio.open(input_file) as dataset:
        ...
        inundation = np.round(100*inundation,0) # [m] -> [cm]        
        ...
        volume_step = np.sum(positive_inundation/100) * resolution
...
def inundation_excl_allowable(input_file, output_file, topo_file, row, water_level):
    # Open height data
...
def fun(polygons):
    ...
    with rasterio.open(config.raster_prep) as src:
        ...
        for idx, polygon in tqdm(polygons.iterrows(), 'Estimate inundation (' + str(len(polygons)) + ' iterations)'):            
            ...
            volume_inun = inundation_excl_allowable(input_file, output_file, topo_file, row, water_level)
