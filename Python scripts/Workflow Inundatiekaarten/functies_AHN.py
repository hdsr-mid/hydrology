...
def fun_preprocess_raster():    
    ...
    ds_Peil = rasterio.open(config.raster_peil, "r+")
    ds_topo = rasterio.open(config.raster_clip, "r+")
    # ds_Peil.nodata = -999
    # print(float(ds_Peil.nodatavals[0]))
    ds_prep = np.where(ds_Peil.read()[0]==-999, ds_topo.read()[0],ds_Peil.read()[0])
    meta = ds_topo.meta
    meta.update({"nodata": -999})
    with rasterio.open(config.raster_prep, 'w', **meta) as dest:
