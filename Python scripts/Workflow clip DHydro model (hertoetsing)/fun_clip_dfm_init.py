...
Doel: 
Verwijder alle regels die horen bij bij:
    - kunstwerken/observation points/profielen (crsloc) die zich bevinden op een branch dat weggeknipt is (niet inbegrepen in network.nc)
    - profiel definities (crsdef) van profielen/kunstwerken dat weggefilterd is (vorig puntje)
    - initial water level definities voor een branch dat weggeknipt is (niet inbegrepen in network.nc)
    - storage nodes die zich bevinden op een branch dat weggeknipt is (niet inbegrepen in network.nc)
    - randvoorwaarden/laterals voor een node/branch dat weggeknipt is (niet inbegrepen in network.nc)
...
def clip_dfm_ini(path,fn_in, groupname):
    ...
    fn_out   = fn_in.replace('.ini','_sel.ini').replace('.ext','_sel.ext')
    ...
    node_id   = xds.network_node_id.values.astype(str)
    node_id   = np.array([x.replace(' ','') for x in node_id ])
    # print(branch_id)
    ...
    if os.path.basename(fn_in) == 'bnd.ext':
        idx_blocks = np.where((np.array(lines)==groupname[0])| (np.array(lines)==groupname[1]))[0] 
    else:
    ...
    with tqdm(total=len(idx_blocks)-1,desc=fn_in) as pbar:
        for i in range(0,len(idx_blocks)):
            ...
            if os.path.basename(fn_in) == 'crsdef.ini':
                ...
                cond       = id_name in IDs                
            elif os.path.basename(fn_in) == 'bnd.ext':
                idx_crit   = row_start + np.where((categories=='nodeId') | (categories=='branchId'))[0][0] # row in block starting with "Id"
                id_name    = lines[idx_crit].split('=')[1].replace(' ','')[:-1] # corresponding string
                id_name    = id_name.split('#')[0]
                cond       = (id_name in node_id) | (id_name in branch_id)
            else:
