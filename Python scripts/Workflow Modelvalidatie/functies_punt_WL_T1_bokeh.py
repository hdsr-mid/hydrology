...
def bokeh_fig(paths, shp_afvoer, shp_points, df_data, vmin, vmax,region_name):  
    metric = 'dWL'
    ...
    # clip by region (otherwise file becomes too big)
    # shp_points  = gpd.clip(shp_points, region)
    ...
    p1 = bokeh.plotting.figure(title='Waterniveau T=1 zomer: RUPROF vs. FEWS-WIS [m]', height=350, width=820)
    ...
    color_bar = ColorBar(color_mapper=color,title='dWL = model (RUPROF) - data (FEWS-WIS) [m]')
    ...
    p3    = bokeh.plotting.figure(title='LOC_ID ' +  shp_points.iloc[0]['ID_left'], height=300, width=1400, x_axis_type='datetime', x_axis_label="time",y_axis_label="WL [m NAP]") # , y_range=(0,1)
